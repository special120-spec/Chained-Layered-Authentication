...
export function devicesRouter(db: Database, chain: ChainStore, rp: RpConfig): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!isValidId(account_id) || !attestationResponse?.response?.clientDataJSON) {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!isValidId(account_id) || !isValidId(add_ticket) || !attestationResponse?.response?.clientDataJSON) {
        return res.status(400).json({ error: "account_id, add_ticket, and attestationResponse required" });
      }
      const ticketRow = db
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!isValidId(account_id) || !isValidId(device_id)) {
        return res.status(400).json({ error: "account_id and device_id required" });
      }
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!isValidId(account_id) || !isValidId(device_id) || !assertionResponse?.response?.clientDataJSON) {
        return res.status(400).json({ error: "account_id, device_id, and assertionResponse required" });
      }
      const device = getDeviceById(db, account_id, device_id);
      if (!device || device.status !== "active") return res.status(404).json({ error: "no such active device" });
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!isValidId(account_id) || !isValidId(rotation_ticket) || !attestationResponse?.response?.clientDataJSON) {
        return res.status(400).json({ error: "account_id, rotation_ticket, and attestationResponse required" });
      }
      const ticketRow = db
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      const { account_id, device_id, assertionResponse } = req.body ?? {};
      if (!isValidId(account_id) || !isValidId(device_id) || !assertionResponse?.response?.clientDataJSON) {
        return res.status(400).json({ error: "account_id, device_id (the target to revoke), and assertionResponse required" });
      }
